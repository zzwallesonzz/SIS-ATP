          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur-xl space-y-6">
            ...
            <div className="pt-4 border-t border-slate-800 space-y-2.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                ...
                <button
                  type="button"
                  id="btn-demo-operador"
                  onClick={() => {
                    const op = usuarios.find((u) => u.perfil === 'Operador') || usuarios[3];
                    if (op) handleSelectDemoUser(op);
                  }}
                  className="bg-slate-950 hover:bg-indigo-950/60 border border-slate-800 hover:border-indigo-500/50 p-2.5 rounded-xl text-left transition-all group"
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[10px] font-bold text-indigo-400 uppercase">Operador</span>
                  ...
                  <span className="text-[10px] text-slate-500 block">Wellington Barbosa</span>
                </button>
                ...
                <button
                  ...
                  onClick={() => {
                    const sup = usuarios.find((u) => u.perfil === 'Supervisor') || usuarios[0];
                    if (sup) handleSelectDemoUser(sup);
                  }}
                  className="bg-slate-950 hover:bg-purple-950/60 border border-slate-800 hover:border-purple-500/50 p-2.5 rounded-xl text-left transition-all group"
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[10px] font-bold text-purple-400 uppercase">Supervisor</span>
                    <BadgeCheck className="w-3 h-3 text-purple-400" />
                  </div>
                  <p className="text-xs font-semibold text-slate-200 truncate group-hover:text-white">
                    carlos.santos
                  </p>
                  <span className="text-[10px] text-slate-500 block">Carlos Santos</span>
                </button>
                ...
                <button
                  ...
                  id="btn-demo-adm"
                  onClick={() => {
                    const adm = usuarios.find((u) => u.perfil === 'ADM') || usuarios[2];
                    if (adm) handleSelectDemoUser(adm);
                  }}
                  className="bg-slate-950 hover:bg-emerald-950/60 border border-slate-800 hover:border-emerald-500/50 p-2.5 rounded-xl text-left transition-all group"
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[10px] font-bold text-emerald-400 uppercase">ADM</span>
                    <Shield className="w-3 h-3 text-emerald-400" />
                  </div>
                  <p className="text-xs font-semibold text-slate-200 truncate group-hover:text-white">
                    admin
                  </p>
                  <span className="text-[10px] text-slate-500 block">Administrador</span>
                </button>
